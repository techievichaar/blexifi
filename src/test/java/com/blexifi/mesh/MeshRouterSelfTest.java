...
public final class MeshRouterSelfTest {
    ...
    private static void tracksOutboxAndAckLifecycle() {
        ...
        DeliveryCoordinator coordinator = new DeliveryCoordinator(store, fixedClock);

        Envelope envelope = Envelope.text("A", "B", "cipher", 6);
        OutboxEntry tracked = coordinator.trackForDelivery(envelope);

        check(coordinator.dueRetries().size() == 1, "new entry should be due immediately");

        OutboxEntry afterRetry = coordinator.markAttemptScheduled(tracked.envelopeId());
        check(afterRetry.attempt() == 1, "attempt should increment");
        check(afterRetry.nextAttemptAtMs() > fixedClock.millis(), "next attempt should be scheduled in future");

        coordinator.markAcked(tracked.envelopeId());
        OutboxEntry acked = store.get(tracked.envelopeId()).orElseThrow();
        check(acked.acked(), "entry should be acked after markAcked");
    }
